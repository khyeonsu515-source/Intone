...
/*
  buildTopicLabelPrompt: 새 주제(사건)가 처음 생성될 때 딱 한 번만 호출해서,
  키워드 나열이 아닌 자연스러운 한 문장 라벨을 짓게 하는 지침입니다.
  기존 주제에 기사가 합류할 때는 이미 지어진 라벨을 그대로 쓰고 다시 묻지
  않습니다(indexArticleLocalTopic 참고) — AI 호출은 기사 수가 아니라 새로
  생기는 사건 수만큼만 발생합니다.
*/
function buildTopicLabelPrompt() {
  return `
너는 여러 개의 키워드와 기사 제목/요약을 보고, 이 사건/이슈를 한 문장(20자
내외)으로 자연스럽게 요약하는 라벨을 짓는 도우미다.
키워드를 나열하지 말고, 사람이 읽기 자연스러운 문장으로 지어라.
응답은 설명 문장 없이 오직 JSON 객체 하나만 반환해라.

형식:
{
  "label": "정부의 종합부동산세 개편안 발표"
}
`.trim();
}
